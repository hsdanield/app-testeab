...
st.set_page_config(
    page_title="Aplicação Teste A/B", page_icon="🅰️🅱️", initial_sidebar_state="expanded"
)
...
def plot_chart(df):
    ...
    chart = (
        ...
        .encode(
            x=alt.X("Group:O", title="Grupo", axis=alt.Axis(labelAngle=0)),
            y=alt.Y("Conversion:Q", title="Taxa de Conversão (%)"),
            opacity="Group:O",
        )
...
st.write(
    """
        # 🅰️🅱️ Aplicação de Teste A/B 
        Carregue o resultado de seus experimentos e veja as metricas de significancia.
        """
)
...
if uploaded_file:
    ...
    st.markdown("### Selecione as colunas para analise")
    with st.form(key="my_form"):
        ...
        conversion = st.multiselect(
            ...
            options = df.columns,
            # help=""
            default = conversion_default
        )   

        if conversion:
            conversions_a = (
                df[[ab[0], conversion[0]]].groupby(ab[0]).agg("sum")[conversion[0]][control]
            )
            conversions_b = (
                df[[ab[0], conversion[0]]].groupby(ab[0]).agg("sum")[conversion[0]][treatment]
            )

        with st.expander("Ajuste parâmetros de teste"):
            st.markdown("### Parâmetros")
            st.radio(
                "Teste de Hipotese",
                options=["One-sized", "Two-sized"],
                index=0,
                key="hypothesis")
    ...
    if not ab or not conversion:
        st.warning("Porfavor selecione os valores de **A/B column** e **Conversion column**.")
        st.stop()
    ...
    st.write("")
    st.write("## Resultados A/B do teste ", name)
    st.write("")
